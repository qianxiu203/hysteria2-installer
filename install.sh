...
EOF
    ...
    # 提取证书 SHA-256 指纹 (pinSHA256)
    PIN_SHA256=""
    if [[ -f "$CERT_FILE" ]]; then
        PIN_SHA256=$(openssl x509 -in "$CERT_FILE" -outform DER 2>/dev/null | openssl dgst -sha256 -binary 2>/dev/null | openssl base64 2>/dev/null || true)
    fi
...
show_client_configs() {
    ...
    if [[ -n "$pin" ]]; then
        echo -e "${YELLOW}服务端证书 SHA-256 指纹 (仅供人工核对，无需写入客户端): ${GREEN}${pin}${PLAIN}"
    fi
    log_info "请在云安全组放行信息页 URL 中的 TCP 端口。"
    log_warn "自签证书模式已在直链中自动附带 insecure=1，客户端扫码即可信任；生产环境推荐使用有效域名证书。"
}
